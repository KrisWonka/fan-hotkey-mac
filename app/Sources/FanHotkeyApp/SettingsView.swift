...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            Section("开机自启") {
                Toggle("登录时自动启动 Hammerspoon + Macs Fan Control", isOn: Binding(
                    get: { autostart },
                    set: { v in autostart = v; Autostart.setEnabled(v) }
                ))
                Text("写一个 LaunchAgent 到 ~/Library/LaunchAgents/com.kriswonka.fanhotkey.autostart.plist。系统启动后两个 app 后台启动，无窗口闪现。")
                    .font(.caption)

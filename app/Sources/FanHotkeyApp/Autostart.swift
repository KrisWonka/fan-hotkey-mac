...
/// 用 LaunchAgent 实现开机自启 Hammerspoon + Macs Fan Control。
/// 走 `~/Library/LaunchAgents/`，纯用户域，不需要 sudo / 不需要任何 TCC 权限。
struct Autostart {
    static let label = "com.kriswonka.fanhotkey.autostart"
    ...
    private static func enable() {
        ...
        <dict>
            ...
            <array>
                ...
                <string>-c</string>
                <string>/usr/bin/open -ga Hammerspoon; /usr/bin/open -gja "Macs Fan Control" --args /minimized</string>
            </array>
        </dict>
        </plist>
        """

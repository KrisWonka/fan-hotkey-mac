#!/usr/bin/env bash
# 一键引导：在全新 macOS 上装好 fan-hotkey-mac
...
bold "[3/5] 安装 Hammerspoon / Macs Fan Control…"
...
# ---- 5. 拉取 fan-hotkey-mac 并跑 install.sh ----
bold "[5/5] 拉取 fan-hotkey-mac 到 $WORKDIR 并安装…"
cd "$WORKDIR"
if [ -d "fan-hotkey-mac/.git" ]; then
  (cd fan-hotkey-mac && git pull --ff-only) || true
  green "  -> 仓库已在，已尝试更新"
else
...
下一步:
  ...
  - Spotlight 搜「Fan Hotkey」开 GUI 调设置
EOF
